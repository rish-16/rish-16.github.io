...
where $$S_X=\{1, 0, 1/2\}$$ is the result of the game ($$1$$ means win, $$0$$ means lose, $$\frac{1}{2}$$ is draw). In fact, if you realise, the lower the expected score $$E_X$$, the higher the "surprise" of player **X** winning the game (i.e., $$S_X=1$$), resulting in a proportionally higher score hike for **X**. Likewise, if it's rather "duh" the player **X** will highly likely win (i.e., a high #$$E_X$$), the lower the score hike for them.
